...
- [ ] (amended 2026-08-24) Compact `src/ui/SetupScreen.tsx` and `src/styles/app.css` so the wide setup page fits below the fixed header within one viewport: shorten the Roster input, remove the card's 720px minimum, reduce vertical spacing/control heights, and place Selection and Camera groups in one settings row while retaining stacked narrow layouts. User will verify directly; do not run automated verification in this iteration.
- [ ] (amended 2026-08-24) Raise the Close up view in `src/race/DecisiveCamera.tsx` from three to five cell pitches above the decisive marble without changing its zero-trail centered tracking, forward sightline, or damping. User will verify directly; do not run automated verification in this iteration.
- [ ] (amended 2026-08-24) Replace the plain obstacle permutation in `src/course/arc.ts` with a seeded deterministic arrangement that preserves the fixed ten-obstacle inventory while preventing the same obstacle type from occupying consecutive obstacle Slots. User will verify directly; do not run automated verification in this iteration.
- [ ] (amended 2026-08-24) Remove Funnel choke from the production Course inventory and `COURSE_MODULES` in `src/course/arc.ts` and `src/course/courseModules.ts`, retain it in Showcase, and preserve ten obstacle Slots by replacing its two occurrences with one Whoops and one Staircase. User will verify directly; do not run automated verification in this iteration.
- [ ] (amended 2026-08-24) Supersede the Close up framing above: return the full three-dimensional Course tangent from `routeForward` in `src/race/cameraTarget.ts` instead of discarding its depth component, damp a three-dimensional forward vector in `src/race/DecisiveCamera.tsx` in place of the planar heading angle so hairpin connectors place the camera behind the marble rather than beside it, and reframe Close up as a true third-person chase positioned behind and slightly above the decisive marble on that tangent with no lateral offset and a forward sightline down the track; keep the Broadcast three-quarter framing. User directed no automated verification in this iteration and will verify in the browser directly; `src/race/DecisiveCamera.test.tsx` already fails two stale Broadcast expectations on this branch and is now further out of date.
- [ ] (amended 2026-08-24) Supersede the Funnel Showcase retention above: reduce the complete Module catalog in `src/modules/registry.ts` to structural Chute plus Diamond field, Whoops, Staircase, and Windmill; delete Steep zigzag, Rumble strip, Friction lanes, Funnel choke, Vortex bowl, their dedicated tests, and the now-unused revolved-geometry emitter; remove obsolete Vortex validation exclusions and current source/domain references while leaving archived specs as historical records. User will verify directly; do not run automated verification in this iteration.
